...
:: Get mpv.exe location (now in the parent directory)
:: %~dp0 gives the directory of the script (e.g., C:\path\to\installer\)
:: ..\ moves one directory up (e.g., C:\path\to\)
set mpv_path=%~dp0..\mpv.exe
if not exist "%mpv_path%" call :die "mpv.exe not found in the parent directory of the script."

:: Get umpvw.exe location (now in the parent directory)
set umpvw_path=%~dp0..\umpvw.exe
if not exist "%umpvw_path%" call :die "umpvw.exe not found in the parent directory of the script."

:: Get mpv-icon.ico location (now in the same directory as the script)
set icon_path=%~dp0mpv-icon.ico
if not exist "%icon_path%" call :die "mpv-icon.ico not found in the same directory as the script."
